...
const CardNav: React.FC<CardNavProps> = ({
  ...
  return (
    <div
      className={`card-nav-container fixed left-1/2 -translate-x-1/2 w-[92%] max-w-[900px] z-[99] top-[1.2em] md:top-[1.5em] ${className}`}
    >
      <nav
        ref={navRef}
      ...
      >
        ...
        <div
          className={`card-nav-content absolute left-0 right-0 top-[60px] bottom-0 p-2.5 flex flex-col items-stretch gap-2 justify-start z-[1] ${
        ...
        >
          {(items || []).slice(0, 3).map((item, idx) => (
            <div
              key={`${item.label}-${idx}`}
              className="nav-card select-none relative flex flex-col gap-2 p-4 rounded-lg min-w-0 flex-[1_1_auto] h-auto min-h-[60px] md:h-full md:min-h-0 md:flex-[1_1_0%] border border-[#534434]/40"
              ref={setCardRef(idx)}
              style={{ backgroundColor: item.bgColor, color: item.textColor }}
            >
              <div className="nav-card-label font-domine font-bold tracking-tight text-[18px] md:text-[20px] text-[#ffc174]">
                {item.label}
              </div>
